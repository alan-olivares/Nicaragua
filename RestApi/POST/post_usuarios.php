...
if($row[0]=='1'){
  ...
  if(strpos($permisos,',3,') !== false){
    if(ISSET($_POST['nombrePerfilC'])){//Perfil a crear
      ...
      $acceso=$_POST["acceso"];
      if($perfil!==""){
        $query="INSERT into CM_Usuario (IdFacultad, IdGrupo,Nombre, Estatus,Clave,Pass,IdPerfil) values ($acceso,'$grupo','$nombre',$activo,'$user',ENCRYPTBYPASSPHRASE('Pims.2011','$passn'),'$perfil')";
      }else{
        $query="INSERT into CM_Usuario (IdFacultad,IdGrupo,Nombre, Estatus,Clave,Pass) values ($acceso,'$grupo','$nombre',$activo,'$user',ENCRYPTBYPASSPHRASE('Pims.2011','$passn'))";
      }
