  ...
  if(strpos($permisos,',9,') !== false){
    ...
    if($caso==='barriles'){
      ...
      foreach ($ids as $consecutivo) {
        ...
        }else if(getDato($conn , "SELECT COUNT(*) from WM_Barrica where IdEstado=2 and Consecutivo='$consecutivo'")!=0){
      ...
      foreach ($ids as $NoSerie) {
        ...
        }else if(getDato($conn , "SELECT COUNT(*) from WM_Tanques where NoSerie='$NoSerie' AND IdEstado=2")!=0){
...
function getDato($conn , $queryCons){
