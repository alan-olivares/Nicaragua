...
if($row[0]=='1'){
  ...
  if(strpos($permisos,',6,') !== false){
    if(ISSET($_POST['bodega']) && ISSET($_POST['year']) && ISSET($_POST['Alcohol']) && ISSET($_POST['Uso']) && ISSET($_POST['Cantidad'])&& ISSET($_POST['tanque'])){//Crear orden
      $bodega=$_POST['bodega'];
      $year=$_POST['year'];
      ...
      $tanque=$_POST['tanque'];
      $tsql = "exec sp_OrdenTrasInsert '$usuario' , '$bodega','$year','$Alcohol','$Uso',$Cantidad,$tanque";
      $stmt = sqlsrv_query( $conn , $tsql);
      if($stmt){
