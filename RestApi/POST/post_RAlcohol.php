  ...
  if(strpos($permisos,',8,') !== false){
    if(ISSET($_POST['operacion'])){//Crear orden
      ...
      if($operacion==='editados'){
        ...
        if(ObtenerCantidad("SELECT * from CM_Tanque where Codigo='$codigo' ",$conn)){
      ...
      if($operacion==='editados'){
        ...
        if($codigoA!==$codigoAN && ObtenerCantidad("SELECT * from CM_Alcohol where Codigo='$codigoA' ",$conn)){
        ...
        if(ObtenerCantidad("SELECT * from CM_Alcohol where Codigo='$codigo' ",$conn)){
